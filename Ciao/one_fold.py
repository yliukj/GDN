...
tf.app.flags.DEFINE_integer('graph_pad_length', 7317, 'graph pad length for training')
...
def fold_cv(cv_index = 2,if_train = FLAGS.train):
  ...
  with tf.Session() as sess:
    ...
    with tf.variable_scope('DownstreamApplication'):
      ...
      learn_rate = tf.train.exponential_decay(lr, global_step, FLAGS.decay_step, 0.96, staircase=True)
      ...
      loss = tf.reduce_mean(tf.losses.mean_squared_error(labels,logit_care))
    ...
    # load data
    u_features,train_u_indices, train_v_indices,train_labels,test_u_indices, test_v_indices,test_labels = load_nci()
    print(len(train_labels))
    train_u_indices, train_v_indices, train_labels = adversarial_noise(u_indices = train_u_indices, v_indices = train_v_indices, labels = train_labels)
    feature_input = csr_matrix((train_labels, (train_u_indices, train_v_indices)), shape=(7317, 1000), dtype = np.float)
    ...
    if if_train == True:
      ...
      for epoch_num in range(num_epochs):
        epoch_loss = 0
        batch_input,topo = (feature_input,structure_input)
